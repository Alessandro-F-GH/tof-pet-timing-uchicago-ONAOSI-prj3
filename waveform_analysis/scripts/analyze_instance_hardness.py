...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Estimate per-event out-of-fold ensemble disagreement using fixed regressors "
            "on d(t)=s1(t)-s2(t)."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("prediction_disagreement"),
        help="Output root (default: ./prediction_disagreement)",
    )
    parser.add_argument(
        "--folds",
        type=int,
...
def _ensemble_disagreement(predictions: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Target-independent disagreement across OOF model predictions.

    D_i = std_m(yhat_mi)

    The population standard deviation is used because the four fixed regressors
    are the complete diagnostic ensemble rather than a sample from a larger set.
    The prediction range is returned as a secondary, easily interpretable spread.
    """
    pred = np.asarray(predictions, dtype=np.float64)
    if pred.ndim != 2:
        raise ValueError(f"Predictions must be [model, event], got {pred.shape}")
    if pred.shape[0] < 2:
        raise ValueError("Ensemble disagreement requires at least two models")
    if not np.all(np.isfinite(pred)):
        raise ValueError("Ensemble disagreement requires finite predictions")

    disagreement = np.std(pred, axis=0, ddof=0)
    prediction_range = np.ptp(pred, axis=0)
    return disagreement, prediction_range
    ...
    *,
    ...
    if kind == "minirocket":
        try:
            ...
            raise ImportError(
                "MiniROCKET disagreement analysis requires aeon; install waveform_analysis/requirements.txt"
            ) from exc
    ...
    raise ValueError(f"Unknown disagreement-analysis model kind: {kind!r}")
...
def _disagreement_plot(
    ...
    *,
    ...
    ax.scatter(x, disagreement, s=10, alpha=0.28, label="training event")
    trend_x, trend_y = _quantile_trend(x, disagreement)
    if trend_x.size:
        ax.plot(
            trend_x,
            trend_y,
    ...
    ax.set_ylabel("OOF ensemble disagreement [ps]")
    ax.set_title(title)
    ...
    *,
    ...
    for model_name in MODEL_SETTINGS:
        model_prediction = _oof_predictions(
            ...
            target,
            folds=folds,
            split_seed=semantic_seed(seed, label, "disagreement_folds"),
    ...
    disagreement, prediction_range = _ensemble_disagreement(prediction_matrix)
    ...
    summaries.append(
        {
            ...
            "mean_disagreement_ps": float(np.mean(disagreement)),
            "median_disagreement_ps": float(np.median(disagreement)),
            "folds": int(folds),
            "training_events": int(training.size),
            "input_samples": int(pair.shape[-1]),
        }
    ...
    for position in range(training.size):
        row: dict[str, Any] = {
            ...
            "prediction_disagreement_ps": float(disagreement[position]),
            "prediction_range_ps": float(prediction_range[position]),
    ...
    event_csv = dataset_output / "prediction_disagreement.csv"
    summary_csv = dataset_output / "model_oof_summary.csv"
    arrays_path = dataset_output / "prediction_disagreement.npz"
    target_plot = dataset_output / f"prediction_disagreement_vs_target.{plot_format}"
    abs_target_plot = (
        dataset_output / f"prediction_disagreement_vs_abs_target.{plot_format}"
    )
    ...
    np.savez_compressed(
        arrays_path,
        target_ps=target,
        prediction_disagreement_ps=disagreement,
        prediction_range_ps=prediction_range,
    ...
    _disagreement_plot(
        ...
        title=f"OOF ensemble disagreement · {title_suffix}",
    )
    _disagreement_plot(
        abs_target_plot,
        np.abs(target),
        ...
        title=f"OOF ensemble disagreement vs target magnitude · {title_suffix}",
    )
