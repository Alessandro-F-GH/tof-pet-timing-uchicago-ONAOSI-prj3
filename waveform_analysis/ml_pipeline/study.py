...
def run_study(config_or_path:dict[str,Any]|str|Path,*,overwrite:bool=False,rebuild_preprocessing:bool=False)->Path:
    ...
    datasets=[_prepare_one(root,config,rebuild_preprocessing,logger) for root in roots]; rows=[]; seed=int(config['validation']['seed']); mode=str(config['mode']); manifest={'schema_version':4,'protocol':'single_mode_raw_split_then_development_fitted_selection_then_calibrated_led_prediction_then_test','mode':mode,'test_used_for_selection':False,'model_selection_metric':'validation_rmse','ml_target':'paired_led_ps - mean_training_led_pair_ps','corrected_residual':'calibrated_led_ps - paired_model_prediction_ps','ctr_metric':'utils_fit_gaussian_fwhm','config':public_config(config),'datasets':{}}
    for dataset in datasets:
        ...
        for model_name,model_config in config['models'].items():
            spec=get_model(model_name); search=search_model(spec,model_config,config,dataset,mode,seed=semantic_seed(seed,name,mode,model_name,'search'),logger=logger); fitted=refit_selected(spec,model_config,dataset,mode,search.best,seed=semantic_seed(seed,name,mode,model_name,'final'),config=config); save_model(spec,fitted,store.model_dir(name,model_name),search.best.candidate); store.save_search(name,model_name,search.as_dict()); final_models[model_name]=FinalModel(fitted,search); rows.append(_selection_row(name,voltage,mode,model_name,search.best.score,search.best.candidate,'validation_rmse')); logger.info('Selected %s/%s | corrected-LED validation RMSE %.6g ps | output clipped to ±%.0f ps | %s',mode,model_name,search.best.score,float(config['ml_output']['max_abs_ps']),search.best.candidate)
        ...
        calibrated=calibrated_led(dataset,mode)
        for stage,indices in (("train",np.asarray(dataset.training,dtype=np.int64)),("test",np.asarray(dataset.test,dtype=np.int64))):
            led=calibrated[indices]; rows.append(_metric_row(config,name,voltage,mode,'led',led,indices.size,semantic_seed(seed,name,mode,'led',stage),logger,stage=stage)); store.save_residuals(name,'led',led,stage=stage)
            if config['cfd']:
