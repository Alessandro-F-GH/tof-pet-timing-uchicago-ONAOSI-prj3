...
def _metric_row(config, name, voltage, mode, method, residual, population_n, seed, logger, stage="test"):
    ...
    return {
        ...
        "core_fwhm_left_ps": float(result.left_half_ps),
        "core_fwhm_right_ps": float(result.right_half_ps),
        "core_fwhm_bin_width_ps": float(result.bin_width_ps),
    ...
    *,
    ...
    for dataset in datasets:
        ...
        for model_name, model_config in config["models"].items():
            ...
            logger.info(
                "Selected dataset=%s | %s/%s | validation RMSE %.6g ps | train target range ±%.6g ps | train used=%d/%d | using selected trained model without refit | output clipped to ±%.0f ps | %s",
                name,
                mode,
                model_name,
                search.best.score,
