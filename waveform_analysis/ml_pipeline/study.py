...
from .stats import bootstrap_ctr_uncertainty, gaussian_ctr
...
def _metric_row(config,name,voltage,mode,method,residual,population_n,seed,stage='test'):
    values=np.asarray(residual,dtype=float); finite=values[np.isfinite(values)]; minimum=int((config.get('fit') or {}).get('min_events',100))
    if finite.size<minimum: raise RuntimeError(f'{name}/{mode}/{method}: only {finite.size} finite {stage} residuals')
    fit=gaussian_ctr(finite,config.get('fit')); nboot=int(config.get('reporting',{}).get('ctr_uncertainty_bootstrap_samples',500)); uncertainty=bootstrap_ctr_uncertainty(finite,nboot,seed,config.get('fit')) if nboot else float('nan')
...
def run_study(config_or_path:dict[str,Any]|str|Path,*,overwrite:bool=False,rebuild_preprocessing:bool=False)->Path:
    ...
    for dataset in datasets:
        ...
        for stage,indices in (("train",np.asarray(dataset.training,dtype=np.int64)),("test",np.asarray(dataset.test,dtype=np.int64))):
            led_mean=float(dataset.manifest['led_training_mean_ps'][family]); led=standard_delta(dataset,mode,'led')[indices]-led_mean; rows.append(_metric_row(config,name,voltage,mode,'led',led,indices.size,semantic_seed(seed,name,mode,'led',stage),stage=stage)); store.save_residuals(name,'led',led,stage=stage)
            if config['cfd']:
                cfd=standard_delta(dataset,mode,'cfd')[indices]-led_mean; rows.append(_metric_row(config,name,voltage,mode,'cfd',cfd,indices.size,semantic_seed(seed,name,mode,'cfd',stage),stage=stage)); store.save_residuals(name,'cfd',cfd,stage=stage)
            anchor=anchor_delta(dataset,mode)[indices]
            for model_name in config['models']:
                final=final_models[model_name]; spec=get_model(model_name); correction,_time,_pair=predict_indices(spec,final.fitted,dataset,mode,indices); residual=anchor+correction-led_mean; rows.append(_metric_row(config,name,voltage,mode,model_name,residual,indices.size,semantic_seed(seed,name,mode,model_name,stage),stage=stage)); store.save_residuals(name,model_name,residual,stage=stage)
