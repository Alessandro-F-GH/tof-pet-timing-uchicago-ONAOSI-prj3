...
def _pair_features(pair: np.ndarray, kernels: tuple[RocketKernel, ...], n_bins: int, statistics: tuple[str, ...]) -> np.ndarray:
    ...
    starts, widths = _bin_geometry(x.shape[-1], n_bins); bins = starts.size; output = np.empty((x.shape[0], len(kernels) * bins * len(statistics)), dtype=np.float32); cursor = 0
    for kernel in kernels:
        response = convolve1d(x, _dense_kernel(kernel), axis=-1, mode="constant", cval=0.0) + np.float32(kernel.bias)
        for statistic in statistics:
            ...
            output[:, cursor:cursor + bins] = pooled[:, 0, :] - pooled[:, 1, :]; cursor += bins
    return output
...
def _cached_features(pair: np.ndarray, config: dict[str, Any], kernels: tuple[RocketKernel, ...], n_bins: int, statistics: tuple[str, ...], signature: tuple[Any, ...]) -> np.ndarray:
    ...
    features = _pair_features(pair, kernels, n_bins, statistics); _FEATURE_CACHE[key] = features; _FEATURE_CACHE.move_to_end(key)
...
def fit(params, train_x, train_target, *, seed, config, validation_x=None, validation_target=None, final_epochs=None):
    ...
    return LocalizedRocketArtifact(model, kernels, n_bins, statistics, scale, signature, metadata)
