...
class OnishiPairedCNN(nn.Module):
...
def fit(
    ...
    *,
    ...
    if x.shape[0] != target.shape[0]:
        raise ValueError("onishi_cnn train_x and train_target must contain the same number of events")
    if x.shape[0] < 1:
...
def explain(artifact: OnishiCNNArtifact, normalized_pair: np.ndarray) -> np.ndarray:
    ...
    if pair.grad is None:
        raise RuntimeError("onishi_cnn XAI gradient is unavailable")
