...
def _best_column(grid,candidates,true_tof,fit,*,minimum_efficiency=0.0,logger=None,label='standard method'):
    ...
    for i,candidate in enumerate(candidates):
        residual=pair_delta(np.asarray(grid[:,:,i],dtype=np.float64))-float(true_tof); finite=np.isfinite(residual); coverage=int(np.count_nonzero(finite)); efficiency=coverage/total; best_observed_efficiency=max(best_observed_efficiency,efficiency)
        if coverage==0: continue
        if efficiency<float(minimum_efficiency):
            if logger is not None: logger.info('%s candidate %.6g rejected | crossing efficiency %.2f%% < %.2f%%',label,float(candidate),100.0*efficiency,100.0*float(minimum_efficiency))
            continue
        try:score=float(gaussian_ctr(residual[finite],fit).ctr_ps)
        except ValueError as exc:
            if logger is not None: logger.warning('%s candidate %.6g Gaussian CTR fit failed | crossing efficiency %.2f%% (%d/%d) | reason=%s | %s',label,float(candidate),100.0*efficiency,coverage,total,exc,format_residual_summary(residual_summary(residual[finite])))
            continue
        key=(score,float(candidate))
        if best is None or key<best[0]: best=(key,float(candidate),score,coverage,efficiency)
    if best is None: raise RuntimeError(f'No {label} candidate satisfies the minimum crossing efficiency {100.0*float(minimum_efficiency):.1f}% and provides a valid Gaussian CTR fit; best observed efficiency={100.0*best_observed_efficiency:.2f}%')
...
def prepare_ml_dataset(preprocessed,config,*,rebuild,logger):
    ...
    if missing_led_index.size: logger.warning('Discarding events without selected LED crossing | discarded=%d/%d | by_family=%s',missing_led_index.size,preprocessed.n_events,missing_by_family)
    ...
    manifest={'format_version':DATASET_FORMAT_VERSION,'fingerprint':dataset_fingerprint(preprocessed,config),'source':preprocessed.manifest['source'],'preprocessed_dir':str(preprocessed.directory),'mode':config['mode'],'true_tof_ps':true_tof,'n_input_events':preprocessed.n_events,'n_events':int(kept_rows.size),'excluded_led_no_crossing':int(missing_led_index.size),'excluded_led_event_index_file':'excluded_led_event_index.npy','excluded_ml_window_exceeds_preprocessing':int(ml_window_index.size),'excluded_ml_window_event_index_file':'excluded_ml_window_event_index.npy','split':{'training':int(training_new.size),'validation':int(validation_new.size),'test':int(test_new.size)},'led_threshold_mV':led_choice,'led_minimum_crossing_efficiency':led_min_eff,'led_development_ctr_ps':led_score,'led_development_coverage':led_development_coverage,'led_development_efficiency':led_development_efficiency,'led_missing_by_family':missing_by_family,'led_training_mean_ps':led_training_mean,'cfd_fraction':cfd_choice,'cfd_development_ctr_ps':cfd_score,'ml_input':config['ml_input'],'normalization':transforms,'diagnostic_examples':diagnostic_examples,'target_definition':'mean_training_led_pair_ps - native_anchor_pair_ps','time_reference':'relative_to_native_sample_closest_to_selected_led_threshold'}; atomic_json(base/'manifest.json',manifest); _ensure_diagnostics(preprocessed,config,manifest); logger.info('ML dataset %s | train=%d validation=%d test=%d | excluded_led=%d | excluded_ml_window=%d | subsampling=%d',Path(preprocessed.manifest['source']).name,training_new.size,validation_new.size,test_new.size,missing_led_index.size,ml_window_index.size,int(config['ml_input'].get('subsampling',1))); return load_prepared_dataset(base)
