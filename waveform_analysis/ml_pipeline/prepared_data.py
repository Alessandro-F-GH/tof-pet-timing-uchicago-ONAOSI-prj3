...
def _best_column(grid,candidates,true_tof,fit):
    ...
    for i,c in enumerate(candidates):
        ...
        try:s=float(gaussian_ctr(residual,fit).ctr_ps)
        except ValueError:continue
        if s<score: score=s; best=float(c)
    if best is None: raise RuntimeError('No standard-method candidate provides a valid Gaussian CTR fit')
    return best,score
...
def prepare_ml_dataset(preprocessed,config,*,rebuild,logger):
    ...
    for family in families:
        dev_led=led_grid(preprocessed,family,development,thresholds); led_choice[family],led_score[family]=_best_column(dev_led,thresholds,true_tof,config.get('fit')); logger.info('Selected %s LED | threshold %.6g mV | development Gaussian CTR %.3f ps',family,led_choice[family],led_score[family]); led_times[family]=led_grid(preprocessed,family,np.arange(preprocessed.n_events),np.asarray([led_choice[family]]))[:,:,0]; anchor_idx[family],anchor_times[family]=anchor_grid(preprocessed,family,led_choice[family])
    ...
    if excluded_event_index.size: logger.warning('ML window exceeds preprocessing window | excluded=%d/%d | event indices saved to %s',excluded_event_index.size,preprocessed.n_events,base/'excluded_ml_window_event_index.npy')
