...
def _best_column(grid,candidates,true_tof,fit):
    ...
    for i,c in enumerate(candidates):
        residual=pair_delta(grid[:,:,i])-float(true_tof)
...
def prepare_ml_dataset(preprocessed,config,*,rebuild,logger):
    ...
    for family in families:
        dev_led=led_grid(preprocessed,family,development,thresholds); led_choice[family],led_score[family]=_best_column(dev_led,thresholds,true_tof,config.get('fit')); led_times[family]=led_grid(preprocessed,family,np.arange(preprocessed.n_events),np.asarray([led_choice[family]]))[:,:,0]; anchor_idx[family],anchor_times[family]=anchor_grid(preprocessed,family,led_choice[family])
