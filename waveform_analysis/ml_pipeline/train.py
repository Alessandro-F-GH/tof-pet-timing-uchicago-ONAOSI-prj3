...
def _fit_once(
    ...
    *,
    ...
    runtime_config["_prediction_max_abs_ps"] = (
    ...
    if initial_artifact is not None:
        kwargs["initial_artifact"] = initial_artifact
    artifact = spec.fit(
        parameters,
    ...
    metadata["output_max_abs_ps"] = (
        None if output_max_abs_ps is None else float(output_max_abs_ps)
    )
    metadata["target_definition"] = (
        "delta_t_led - delta_anchor_shift - true_tof - calibration_bias"
    )
    return FittedModel(
        artifact,
        metadata,
...
def predict_model(spec: ModelSpec, fitted: FittedModel, pair: np.ndarray) -> np.ndarray:
    values = np.asarray(
        spec.predict(fitted.artifact, np.asarray(pair, dtype=np.float32)),
        dtype=np.float64,
    )
    if fitted.output_max_abs_ps is not None:
        values = np.clip(
            values,
            -float(fitted.output_max_abs_ps),
            float(fitted.output_max_abs_ps),
        )
    return values
...
def refit_selected(spec, model_config, dataset, mode: str, selected, *, seed: int, config=None) -> FittedModel:
    development = np.asarray(dataset.development, dtype=np.int64)
    x = waveform_view(dataset, mode, development).materialize()
    target = model_target(dataset, mode)[development]
    epochs = selected.metadata.get("best_epoch") if selected.metadata else None
    initial = (
        selected.artifact.artifact
        if spec.name == "cnn" and selected.artifact is not None
        else None
    )
    output_limit = None if config is None else float(config["ml_output"]["max_abs_ps"])
    fitted = _fit_once(
        spec,
        model_config,
        selected.candidate,
