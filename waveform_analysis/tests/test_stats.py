...
class GaussianCTRTests(unittest.TestCase):
    ...
    def test_bootstrap_uses_gaussian_estimator(self):
        rng = np.random.default_rng(4); _fit, uncertainty = bootstrap_ctr_ps(rng.normal(0.0, 30.0, 4000), 25, 7); self.assertTrue(np.isfinite(uncertainty)); self.assertGreater(uncertainty, 0.0)
