...
class DatasetPreparationTests(unittest.TestCase):
    def _data(self):
        ...
        return PreprocessedData(Path('.'), {"source":"synthetic.root","fingerprint":"x","materialized_window_ns":{"before":4.0,"after":20.0}}, np.arange(n), np.array([0,0,0,0,1,1],dtype=np.int8), np.ones(n)*45, waves, None, starts, None, intervals, None, rising_start, None, rising_stop, None)
    def test_led_anchor_uses_native_sample_closest_to_threshold(self):
        ...
        anchor_index=anchor_grid(data,"energy",10.5); self.assertEqual(anchor_index[0,0],10); self.assertEqual(anchor_index[0,1],5)
    def test_led_threshold_is_relative_to_event_baseline(self):
        n, length = 2, 32
        ...
        waves[0, 1] = 2.0 * ramp
        waves[1, 0] = ramp + 7.0
        waves[1, 1] = 2.0 * ramp - 4.0
        starts = np.zeros((n, 2))
        intervals = np.full((n, 2), 1e-9)
        ...
        data = PreprocessedData(
            Path("."),
            {
                ...
                "fingerprint": "x",
                "materialized_window_ns": {"before": 4.0, "after": 20.0},
            },
            np.arange(n),
        ...
        np.testing.assert_allclose(grid[0], grid[1], rtol=0.0, atol=1e-6)
