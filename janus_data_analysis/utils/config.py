...
def _apply_defaults(cfg: dict[str, Any]) -> None:
    ...
    inference.setdefault("maximum_deviation_ns", None)

    streaming = cfg.setdefault("preprocessing", {}).setdefault("streaming_physical_time", {})
    ...
    reconstruction = cfg.setdefault("preprocessing", {}).setdefault("pulse_reconstruction", {})
...
def load_config(path: str | Path, root: str | Path) -> dict[str, Any]:
    ...
    cfg["paths"]["input_dir"] = str((project_root / cfg["paths"]["input_dir"]).resolve())
    cfg["paths"]["output_dir"] = str((project_root / cfg["paths"]["output_dir"]).resolve())
    return cfg


def validate_config(cfg: dict[str, Any]) -> None:
    required = (
        "paths", "files", "runs", "tasks", "preprocessing", "matching_model",
        "channels", "peak_selection", "alignment_filter", "fit", "plots",
        "thresholds", "analysis_output",
    )
    ...
    for key in (
        "streaming_pulse_decode", "candidate_preprocessing", "energy_selection",
        "matching_training", "matching_model", "preprocessing", "selection", "fit",
    ):
    ...
    streaming = preprocessing.get("streaming_physical_time")
    if not isinstance(streaming, dict):
        raise ConfigError("preprocessing.streaming_physical_time is required")
    for flag in ("enabled", "require_same_board"):
        if not isinstance(streaming.get(flag), bool):
            raise ConfigError(f"preprocessing.streaming_physical_time.{flag} must be true or false")
    if not streaming["enabled"]:
        raise ConfigError("STREAMING physical-time preprocessing must be enabled")
    _positive_number(streaming.get("energy_leading_pair_window_ns"), "preprocessing.streaming_physical_time.energy_leading_pair_window_ns", 1e-12)
    ...
    if not isinstance(reconstruction.get("deduplicate_exact_edges"), bool):
        raise ConfigError("preprocessing.pulse_reconstruction.deduplicate_exact_edges must be true or false")
    for flag in ("fallback_when_tot_missing", "fallback_when_tot_inconsistent"):
        if not isinstance(reconstruction.get(flag), bool):
            raise ConfigError(f"preprocessing.pulse_reconstruction.{flag} must be true or false")
    _positive_number(reconstruction.get("tot_lead_match_tolerance_ns"), "preprocessing.pulse_reconstruction.tot_lead_match_tolerance_ns", 0.0)
    minimum_tot = _positive_number(reconstruction.get("minimum_tot_ns"), "preprocessing.pulse_reconstruction.minimum_tot_ns", 0.0)
    maximum_tot = _positive_number(reconstruction.get("maximum_energy_tot_ns"), "preprocessing.pulse_reconstruction.maximum_energy_tot_ns", 1e-12)
    if minimum_tot >= maximum_tot:
        raise ConfigError("minimum_tot_ns must be smaller than maximum_energy_tot_ns")

    for key in ("signal_a", "time_a", "signal_b", "time_b"):
        _positive_int(cfg["channels"].get(key), f"channels.{key}")
    if len({int(cfg["channels"][key]) for key in ("signal_a", "time_a", "signal_b", "time_b")}) != 4:
    ...
    outlier_filter = matching["average_delay"].get("outlier_filter")
    ...
    if not isinstance(outlier_filter.get("enabled"), bool):
        raise ConfigError("matching_model.average_delay.outlier_filter.enabled must be true or false")
    _positive_number(outlier_filter.get("z_threshold"), "matching_model.average_delay.outlier_filter.z_threshold", 1e-9)
    _positive_int(outlier_filter.get("max_iterations"), "matching_model.average_delay.outlier_filter.max_iterations", 1)
    _positive_number(outlier_filter.get("minimum_scale_lsb"), "matching_model.average_delay.outlier_filter.minimum_scale_lsb", 1e-12)
    inference = matching["inference"]
    _positive_number(inference.get("candidate_window_ns"), "matching_model.inference.candidate_window_ns", 1e-12)
    if float(training["window_ns"]) > float(inference["candidate_window_ns"]):
        raise ConfigError("matching_model.training.window_ns must be <= matching_model.inference.candidate_window_ns")
    if inference.get("maximum_deviation_ns") is not None:
        _positive_number(inference["maximum_deviation_ns"], "matching_model.inference.maximum_deviation_ns", 0.0)
    ...
    if str(output.get("diagnostic_mode", "")).lower() not in {"compact", "debug", "summary"}:
        raise ConfigError("analysis_output.diagnostic_mode must be compact, debug or summary")

    peak = cfg["peak_selection"]
    _positive_int(peak.get("min_events"), "peak_selection.min_events", 2)
    _positive_int(peak.get("kde_grid_points"), "peak_selection.kde_grid_points", 256)
    _positive_number(peak.get("kde_bandwidth_factor"), "peak_selection.kde_bandwidth_factor", 1e-9)
    _positive_number(peak.get("min_peak_height_fraction"), "peak_selection.min_peak_height_fraction", 0.0)
    _positive_number(peak.get("left_sigma_multiplier"), "peak_selection.left_sigma_multiplier", 0.0)
    _positive_number(peak.get("right_sigma_multiplier"), "peak_selection.right_sigma_multiplier", 0.0)
    ...
    if str(alignment.get("method", "")).lower() not in {"robust_mad", "standard"}:
        raise ConfigError("alignment_filter.method must be robust_mad or standard")
    _positive_number(alignment.get("z_threshold"), "alignment_filter.z_threshold", 1e-9)
    _positive_number(alignment.get("minimum_scale_lsb"), "alignment_filter.minimum_scale_lsb", 1e-9)
    ...
    _positive_number(led_rejection.get("zscore_limit"), "fit.led_outlier_rejection.zscore_limit", 1e-9)

    plots = cfg["plots"]
    for key in ("matching_train", "matching_total", "peak_selection", "timing_fit"):
        item = plots.get(key)
    ...
    _positive_int(plots.get("max_histogram_bins"), "plots.max_histogram_bins", 10)
    _positive_int(plots.get("matching_curve_points"), "plots.matching_curve_points", 100)

    if str(cfg["thresholds"].get("consistency", "")).lower() not in {"error", "warning"}:
...
def stage_config(cfg: dict[str, Any], stage: str) -> dict[str, Any]:
    if stage == "streaming_pulse_decode":
        return {
            ...
            "streaming_physical_time": cfg["preprocessing"]["streaming_physical_time"],
            "pulse_reconstruction": cfg["preprocessing"]["pulse_reconstruction"],
            "training_window_ns": cfg["matching_model"]["training"]["window_ns"],
            "candidate_window_ns": cfg["matching_model"]["inference"]["candidate_window_ns"],
            "version": 4,
        }
    ...
    if stage == "energy_selection":
        return {"channels": cfg["channels"], "peak_selection": cfg["peak_selection"], "analysis_output": _stage_output_config(cfg), "version": 2}
    if stage == "matching_training":
        return {"channels": cfg["channels"], "training": cfg["matching_model"]["training"], "analysis_output": _stage_output_config(cfg), "version": 8}
    if stage == "matching_model":
        return {"method": cfg["matching_model"]["method"], "average_delay": cfg["matching_model"]["average_delay"], "minimum_samples": cfg["matching_model"]["training"]["minimum_samples"], "version": 5}
    if stage == "preprocessing":
        return {"channels": cfg["channels"], "inference": cfg["matching_model"]["inference"], "analysis_output": _stage_output_config(cfg, supports_summary=True), "version": 11}
    if stage == "selection":
        return {"channels": cfg["channels"], "alignment_filter": cfg["alignment_filter"], "analysis_output": _stage_output_config(cfg), "version": 4}
    if stage == "fit":
        return {"fit": cfg["fit"], "analysis_output": _stage_output_config(cfg), "version": 2}
    if stage.startswith("plot_"):
        versions = {"plot_matching_train": 8, "plot_matching_total": 9, "plot_peak_selection": 3, "plot_timing_fit": 3}
        ...
        if stage == "plot_matching_train":
            result["delay_window_ns"] = cfg["matching_model"]["training"]["window_ns"]
        if stage == "plot_matching_total":
            result["delay_window_ns"] = cfg["matching_model"]["inference"]["candidate_window_ns"]
        return result
