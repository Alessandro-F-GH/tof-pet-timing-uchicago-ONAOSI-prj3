...
This section summarizes the analysis path shared by the waveform-based machine-learning models. The objective is to keep event definition, preprocessing, timing target, waveform windows, and blind-test evaluation fixed so that the main experimental variable is the model representation.
...
Fitted selection quantities are determined without using the blind-test population. The same persisted split identities are reused by the dense MLP, locally connected MLP, and Onishi model studies, and the run-comparison utility checks the blind event identities before model-output correlations are computed.
...
The two detector waveforms are synchronized to the selected LED reference and materialized on the native sampling grid. For the dense MLP and Onishi CNN, a temporal mask derived from the training split removes samples that are effectively constant in both detector channels and is then applied unchanged to validation and blind-test inputs. The locally connected MLP intentionally bypasses this mask and retains the complete configured temporal grid so every receptive field contains consecutive native samples.
...
\end{align}
The default temporal subsampling is one. Dense-MLP and Onishi inputs remain on the native acquisition grid apart from the train-derived constant-sample mask, while the locally connected model keeps the full configured grid.
...
The final models use two development policies: the dense and locally connected MLPs are tunable models, while the Onishi CNN is a fixed literature reference.

For the antisymmetric MLP variants, candidate hyperparameters are fitted using the training split only when more than one candidate is configured. An internal holdout carved from the training split is used for epoch-level early stopping, while the externally persisted validation split ranks the candidate configurations by validation CTR. After selection, the search checkpoint is discarded and a fresh final model is fitted on the complete development population. During this final fit, the MLP reserves the configured internal holdout from the development population for early stopping. If only one candidate is configured, validation-based model selection is skipped and the final development fit is performed directly.

The Onishi CNN exposes one fixed reference configuration and therefore performs no validation-based hyperparameter selection. Its final reference network is fitted once on the complete development population, i.e. training plus validation, and is then evaluated on the blind-test split. For models that use the temporal sample mask, it remains derived from the training split only.

This asymmetry in the fitting protocol must be stated explicitly when interpreting the model comparison: the Onishi model is treated as a fixed reference architecture, while the MLP variants use validation only for hyperparameter selection when selection is actually required. Both final models are fitted on the development population before blind-test evaluation.
