...
Leading-edge discrimination (LED) provides the common timing reference. The threshold-crossing time is obtained by linear interpolation on the rising edge. A fixed inter-channel timing offset is estimated from the training population:
\begin{equation}
    \widehat{C}_{12}
    =
...
For the antisymmetric MLP, candidate hyperparameters are fitted using the training split only. An internal holdout carved from the training split is used for epoch-level early stopping and random-restart selection. The externally persisted validation split is then used only to rank the candidate hyperparameter configurations by validation CTR. The selected trained checkpoint is frozen and evaluated directly on blind data; there is no refit after model selection.
...
This asymmetry in the fitting protocol must be stated explicitly when interpreting the model comparison: the Onishi model is treated as a fixed reference architecture, while the MLP is a validation-selected proposed model.
