...
\begin{itemize}
    ...
    \item \textbf{MLP selection hierarchy:} an internal holdout controls epoch-level stopping, while the external validation split ranks hyperparameter candidates only when more than one candidate is configured. The selected parameters are then refitted on the full development population before blind evaluation.
